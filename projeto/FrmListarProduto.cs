...
namespace projeto
{
    public partial class FrmListarProduto : Form
    {
        ...
        private void FrmListarProduto_Load(object sender, EventArgs e)
        {
            ...
            int registro;//registro da tabela
            int x = 10, y = 10;// posição do formulario
            for (registro = 0; registro < dt.Rows.Count; registro++)
            {
                //painel para os produto
                ...
                qtde.Name = "qtde";
                qtde.Location = new Point(35, 170);
                CheckBox selecionar = new CheckBox();
                selecionar.Text = dt.Rows[registro][0].ToString();
                selecionar.Location = new Point(15, 10);
                selecionar.Click += new EventHandler((sender1, e1) => selecionarClick(sender1, e1, codigoproduto.Text, nome.Text, preco.Text, qtde.Text));
        ...
        private void selecionarClick(object sender, EventArgs e, string id, string nome, string preco, string quantidade)
        {
            int codigoId = Convert.ToInt32(id);
            decimal valor = Convert.ToDecimal(preco);
            int qtde = Convert.ToInt32(quantidade);
            CheckBox cb = (CheckBox)sender;// pegar o checkbox
            if (cb.Checked)
            {
            ...
            else // produto foi desmarcado
            {
                ...
                if (itemExiste != null)
                {
                    ...
                    {
                        CarrinhoAtual.Remove(itemExiste);//remove o item da lista se não existir
                    }
                }
            }
        }
